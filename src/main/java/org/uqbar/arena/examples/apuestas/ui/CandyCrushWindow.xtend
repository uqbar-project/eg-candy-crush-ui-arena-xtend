...
import org.uqbar.arena.examples.apuestas.domain.Caramelo
		...
		modelObject.tablero.celdas.forEach[celda|
			new Panel(tableroPanel, celda) => [
				new Button(it) => [
					width = 40
					heigth = 40
					bindBackgroudToTransformer("contenido.color", [c| 
						switch(c) {
...
//					bindCaptionToProperty("seleccionada").transformer = [m| if (m) "   X   " else "      " ].toViewTransformer(typeof(Boolean))
